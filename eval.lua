...
cmd:option('-url_link', '', 'Url of link')
...
--   _G.COUNT =  v
-- end
--   local f = assert(io.open('count.txt', 'wb'))
--   f:write(_G.COUNT + 1)
--   f:close()
--   return _G.COUNT
  return 1
...
local body, code = http.request(opt.url_link)
if not body then error(code) end

local f = assert(io.open('images/Image' .. increment() .. '.jpg', 'wb'))
f:write(body)
f:close()
